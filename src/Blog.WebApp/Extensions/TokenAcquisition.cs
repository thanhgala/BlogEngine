...
//        /// <param name="tokenCacheProvider"></param>
...
//            var application = CreateApplication(context, context.User, null, CookieAuthenticationDefaults.AuthenticationScheme);
...
//        /// </code>
//        /// </example>
//        public async Task AddAccountToCacheFromJwt(JwtBearer.TokenValidatedContext tokenValidatedContext, IEnumerable<string> scopes)
//        {
//            if (tokenValidatedContext == null)
//                throw new ArgumentNullException(nameof(tokenValidatedContext));

//            await AddAccountToCacheFromJwt(scopes,
...
//        /// </code>
//        /// </example>
//        public async Task AddAccountToCacheFromJwt(OpenIdConnect.TokenValidatedContext tokenValidatedContext, IEnumerable<string> scopes = null)
//        {
//            if (tokenValidatedContext == null)
//                throw new ArgumentNullException(nameof(tokenValidatedContext));
...
//            var app = CreateApplication(context.HttpContext, user, context.Properties, CookieAuthenticationDefaults.AuthenticationScheme);
...
//            var currentUri = UriHelper.BuildAbsolute(request.Scheme, request.Host, request.PathBase, _configuration["AzureAd:CallbackPath"] ?? string.Empty);

//            var credential = new ClientCredential(_configuration["AzureAd:ClientSecret"]);

//            TokenCache userTokenCache = _tokenCacheProvider.GetCache(httpContext, claimsPrincipal, authenticationProperties, signInScheme);

//            //object calcInstance = Activator.CreateInstance(c);

//            //PropertyInfo numberPropertyInfo = c.GetProperty("AccessTokenCount");

//            //var value = numberPropertyInfo.GetValue(c, null);

//            string authority = $"{_configuration["AzureAd:Instance"]}{_configuration["AzureAd:TenantId"]}/v2.0";

//            var app = new ConfidentialClientApplication(_configuration["AzureAd:ClientId"], authority, currentUri, credential, userTokenCache, null);
...
//            try
//            {
//                var result = await application.AcquireTokenSilentAsync(scopes.Except(_scopesRequestedByMsalNet),account);
...
//        /// <param name="httpContext"></param>
//        private async Task AddAccountToCacheFromJwt(IEnumerable<string> scopes, JwtSecurityToken jwtToken, AuthenticationProperties properties, ClaimsPrincipal principal, HttpContext httpContext)
//        {
//            try
//            {
...
//                var result = await application.AcquireTokenOnBehalfOfAsync(scopes.Except(_scopesRequestedByMsalNet), userAssertion);
//            }
//            catch (MsalException ex)
//            {
//                string message = ex.Message;
